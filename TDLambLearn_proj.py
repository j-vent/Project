...
weight = (0.2 * np.random.random(numfeat)) - 0.1 #^
...
while(time <= numSteps): #keyboard interrupt
    with open("reward.txt", "a") as f:
        ...
        #print("delta", delta)
        reward_base += delta * alpha_reward #where is alpha_reward initialized?
        #print("elig", elig, "state", state)
        elig = lamb * elig 
        elig[state]= 1
        ...
        #print("reward", reward)
        reward_base += reward/(time + 1)
        
        
       
        print("action", action, "reward",reward, "probabilities", E)
        f.write(str(time)+','+str(reward) +'\n')
